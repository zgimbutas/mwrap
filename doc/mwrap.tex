...
\title{\mwrap\ User Guide\\ Version 1.4}
...
\begin{verbatim}
  mwrap [-mex outputmex] [-cppmex outputcppmex] [-oct outputoct]
        [-fmex outputfmex]
        [-m output.m] [-c outputmex.c] [-mb]
        [-list] [-catch] [-i8] [-c99complex] [-cppcomplex] [-gpu] [-v]
        infile1 infile2 ...
\end{verbatim}
where
\begin{itemize}
...
\item {\tt -cppmex} specifies the name of the MATLAB C++ MEX API function
  to generate.  The generated code uses the R2018a+ C++ MEX API
  ({\tt MexFunction} class) and should be compiled with
  {\tt mex -R2018a} or later.  This flag is an alternative to
  {\tt -mex}/{\tt -c}, producing a C++ gateway instead of a C MEX file.

\item {\tt -oct} specifies the name of the Octave oct-file function to
  generate.  The generated code uses {\tt DEFUN\_DLD} and should be
  compiled with {\tt mkoctfile}.  This flag is an alternative to
  {\tt -mex}/{\tt -c}, producing a native Octave gateway instead of a
  C MEX file.

\item {\tt -fmex} specifies the name of the Fortran MEX function to
  generate.  The generated code is free-form Fortran~90 ({\tt .f90})
  using {\tt iso\_c\_binding} and {\tt bind(C)} for direct C interop
  with the MATLAB/Octave MEX API.  This produces a pure Fortran
  gateway with no C bridge file, no name mangling macros, and no
  preprocessor.  All wrapped functions must be declared with the
  {\tt FORTRAN} keyword.  Compile with {\tt mex} (MATLAB) or
  {\tt mkoctfile --mex} (Octave).  This flag is mutually exclusive
  with {\tt -oct} and {\tt -cppmex}.
...
\item {\tt -i8} tells \mwrap\ to convert {\tt int}, {\tt long}, {\tt
  ...
  For the {\tt -fmex} backend, {\tt -i8} causes all integers to be
  emitted as {\tt integer(8)}, matching gfortran's
  {\tt -finteger-4-integer-8} promotion.
...
\item {\tt -v} tells \mwrap\ to include verbose comments in the
  generated {\tt .m} stubs.  When enabled, each dispatch line
  {\tt mex\_id\_ = N} is preceded by a {\tt \%\%\%} comment showing
  the human-readable function signature for that ID.
...
\subsection{Zero-copy array passing}
\label{nocopy-section}

By default, \mwrap\ copies array data between MATLAB and the
wrapped C/C++ function: input arrays are copied from the MATLAB
matrix into a temporary buffer, and output arrays are copied back.
For large arrays this overhead can be significant.

The {\tt nocopy} modifier eliminates these copies by passing
pointers directly into (or out of) the MATLAB array storage.
It can be combined with {\tt input}, {\tt output}, or {\tt inout}:
\begin{verbatim}
  # func(nocopy input double[n] x, nocopy output double[n] y, int n);
\end{verbatim}

\begin{itemize}
\item {\tt nocopy input} --- the wrapped function receives a pointer
  directly into the MATLAB input array's data.  The data must not be
  modified by the callee.

\item {\tt nocopy output} --- \mwrap\ pre-allocates the MATLAB output
  array and passes a pointer to its data buffer.  The callee writes
  results directly into the output array, avoiding a copy.

\item {\tt nocopy inout} --- the input array's data pointer is passed
  directly and the same array is returned as output.  The callee may
  modify the data in place.

  \emph{Note:} as of version 1.4.1, {\tt nocopy inout} falls back to
  the safe copy path in codegen.  Modifying an input {\tt mxArray}'s
  data in place is unsafe under MATLAB's copy-on-write semantics
  (aliased arrays share storage; an in-place write silently corrupts
  every alias).  The declaration is still accepted so existing
  interfaces keep compiling, but the wrapped function receives a
  writable copy rather than a pointer into the caller's array.
\end{itemize}

The {\tt nocopy} modifier is supported for numeric array types
({\tt double[]}, {\tt float[]}, {\tt int[]}, etc.) in all backends.
It should only be used when the wrapped function's memory access
pattern is compatible with direct access to MATLAB/Octave storage.
...
It is possible to use \mwrap\ to bind FORTRAN functions.  By default
(with {\tt -mex} or {\tt -cppmex}), the generated MEX gateway is a
C/C++ file that calls Fortran routines via {\tt extern "C"} and
name-mangling macros.  Alternatively, the {\tt -fmex} flag generates
a pure Fortran~90 gateway using {\tt iso\_c\_binding}, eliminating all
C code and name-mangling issues (see Section~2).

FORTRAN bindings are specified using the {\tt FORTRAN} keyword
immediately before a function name; for example:
\begin{verbatim}
...
\subsection{Pure Fortran gateway ({\tt -fmex})}

The {\tt -fmex} backend generates a pure Fortran~90 gateway file
({\tt .f90}) that uses {\tt iso\_c\_binding} and {\tt bind(C)} to
interface directly with the MATLAB/Octave MEX API.  Because the
gateway is itself Fortran, there is no name mangling, no
{\tt extern "C"}, and no need for a C compiler.  The name-mangling
flags ({\tt -DMWF77\_CAPS}, etc.)\ are not needed.

The {\tt -fmex} backend supports the following types: {\tt double},
{\tt float}, {\tt int}, {\tt int32\_t}, {\tt int64\_t}, {\tt dcomplex},
{\tt fcomplex}, and {\tt cstring}.  Objects, {\tt mxArray}, and GPU
arrays are not supported.  All functions must use the {\tt FORTRAN}
keyword.

Example usage:
\begin{verbatim}
  mwrap -cppcomplex -fmex nufftmex -c nufftmex.f90 -mb nufft.mw
  mkoctfile --mex nufftmex.f90 nufft_source.f
\end{verbatim}
or for MATLAB:
\begin{verbatim}
  mex nufftmex.f90 nufft_source.f
\end{verbatim}
