...
\vspace{5mm} \\
  ...
  iospec     & {\tt input} \gOR\ {\tt output} \gOR\ {\tt inout} \gOR\
               {\tt cinput} \gOR\ {\tt coutput} \gOR\ {\tt cinout} \gOR\
               $\epsilon$ \\
...
\subsection{Zero-copy array passing ({\tt cinput}, {\tt coutput}, {\tt cinout})}
\label{nocopy-section}

By default, \mwrap\ copies array data between MATLAB's internal
storage and temporary C buffers.  For numeric types whose MATLAB
class matches the C type exactly (e.g.\ {\tt double} to {\tt
  mxDOUBLE\_CLASS}, {\tt float} to {\tt mxSINGLE\_CLASS}), this copy
is unnecessary and can be avoided using the zero-copy keywords {\tt
  cinput}, {\tt coutput}, and {\tt cinout}.  These provide direct
pointer access to the underlying MATLAB array data without any
intermediate allocation or copy.

\begin{itemize}
\item {\tt cinput} --- provides a read-only direct pointer to the
  input array data.  The MATLAB class of the input is validated at
  runtime; a mismatch produces an error such as {\tt 'Invalid array
    argument, mxDOUBLE\_CLASS expected'}.

\item {\tt coutput} --- allocates the output {\tt mxArray} directly
  (via {\tt mxCreateNumericMatrix}) and provides a writable pointer
  into it.  No temporary buffer is used and no copy-back is needed.
  The output dimensions must be specified explicitly, as with ordinary
  {\tt output} arrays.

\item {\tt cinout} --- combines {\tt cinput} and {\tt coutput}: the
  input array pointer is used directly and the same {\tt mxArray} is
  returned as output ({\tt plhs[i] = prhs[j]}).  The C function
  modifies the data in-place.  Note that this bypasses MATLAB's
  copy-on-write semantics, so the caller should not rely on the
  original variable remaining unmodified.
\end{itemize}

The following types are supported for zero-copy access:
\begin{center}
\begin{tabular}{lll}
  C type       & MATLAB class       & Accessor \\
  \hline
  {\tt double}    & {\tt mxDOUBLE\_CLASS} & {\tt mxGetDoubles} \\
  {\tt float}     & {\tt mxSINGLE\_CLASS} & {\tt mxGetSingles} \\
  {\tt int32\_t}  & {\tt mxINT32\_CLASS}  & {\tt mxGetInt32s} \\
  {\tt int64\_t}  & {\tt mxINT64\_CLASS}  & {\tt mxGetInt64s} \\
  {\tt uint32\_t} & {\tt mxUINT32\_CLASS} & {\tt mxGetUint32s} \\
  {\tt uint64\_t} & {\tt mxUINT64\_CLASS} & {\tt mxGetUint64s} \\
  {\tt dcomplex}  & {\tt mxDOUBLE\_CLASS} (complex) & {\tt mxGetComplexDoubles} \\
  {\tt fcomplex}  & {\tt mxSINGLE\_CLASS} (complex) & {\tt mxGetComplexSingles} \\
\end{tabular}
\end{center}
Types without an unambiguous MATLAB class mapping ({\tt int}, {\tt
  long}, etc.) cannot be used with these keywords; use {\tt -i8} to
promote to fixed-width types, or explicitly use {\tt int32\_t}/{\tt
  int64\_t}.

Complex zero-copy ({\tt dcomplex}, {\tt fcomplex}) requires
interleaved complex storage, available in MATLAB R2018a and later when
compiled with {\tt -R2018a}.  On systems that use split-complex
storage (older MATLAB, or GNU Octave versions without interleaved
complex support), a runtime error is emitted.

Example usage:
\begin{verbatim}
$[
  void scale_array(const double* in, double* out, int n) {
...
@function y = scale(x)
  n = numel(x);
  # scale_array(cinput double[n] x, coutput double[n] y, int n);
\end{verbatim}
